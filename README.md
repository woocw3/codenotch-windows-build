...
이 저장소는 [원본 Codenotch](https://github.com/vinzdg/codenotch)의 Windows 64비트용 비공식 빌드(v0.3.0)를 배포합니다. 원본 커밋은 [`9156c615bcd75eb50683f047d733bc39738dd7c3`](https://github.com/vinzdg/codenotch/commit/9156c615bcd75eb50683f047d733bc39738dd7c3)이며, 원본의 MIT 라이선스는 [LICENSE](LICENSE)에 포함했습니다. 설치 프로그램은 [빌드 워크플로](.github/workflows/build-windows.yml)가 Windows 러너에서 만들며, 빌드에 쓴 커밋과 체크섬은 [downloads/BUILD-INFO.txt](downloads/BUILD-INFO.txt)에 기록됩니다.
...
[ZIP 파일](https://raw.githubusercontent.com/woocw3/codenotch-windows-build/main/downloads/Codenotch-Windows-v0.3.0-x64.zip)에도 설치 프로그램이 들어 있습니다. 설치 없이 사용하려면 ZIP을 풀고 `codenotch.exe`를 실행하세요. 이때 `codenotch-hook.exe`는 같은 폴더에 두세요. 설치 프로그램에도 두 파일이 함께 들어 있습니다.
...
3. **조회가 잠시 멈춰 있을 수 있습니다.** 사용량 API가 429를 돌려주면 Codenotch는 Retry-After만큼 조회를 멈추며, 이 대기 시간은 껐다 켜도 유지됩니다. 2번으로 토큰을 유효하게 만든 뒤 새로고침하는 것이 가장 빠릅니다.
...
현재 배포본에는 만료 직전 토큰을 독립 실행형 `claude` CLI로 자동 갱신하는 수정(원본 커밋 [`27e4b34`](https://github.com/vinzdg/codenotch/commit/27e4b34))이 들어 있습니다. 따라서 2번은 보통 필요 없고, CLI를 찾지 못했거나 토큰이 이미 만료된 경우에만 하면 됩니다.

이 설치 프로그램은 전자서명이 없어 Windows에서 게시자 경고가 표시될 수 있습니다. 패키지에 계정 인증정보나 개인 설정은 포함하지 않았습니다.

## 파일 확인용 SHA-256

- 설치 프로그램: `C259AC6B62474670BD4B9A4812153920AA4090528ADA73D81F471285CCB97429`
- ZIP: `332E0178A237B301C950F0F0A1C1295D34C86477D177E3F566931AA886B4B09E`
...
This repository distributes an unofficial Windows x64 build of [vinzdg/codenotch](https://github.com/vinzdg/codenotch), v0.3.0, built from upstream commit [`9156c615bcd75eb50683f047d733bc39738dd7c3`](https://github.com/vinzdg/codenotch/commit/9156c615bcd75eb50683f047d733bc39738dd7c3). The original project is MIT-licensed; see [LICENSE](LICENSE). The installer is produced on a Windows runner by the [build workflow](.github/workflows/build-windows.yml), which records the commit it built and the checksums in [downloads/BUILD-INFO.txt](downloads/BUILD-INFO.txt).
...
The [ZIP archive](https://raw.githubusercontent.com/woocw3/codenotch-windows-build/main/downloads/Codenotch-Windows-v0.3.0-x64.zip) also contains the Setup executable and portable binaries. For portable use, extract the ZIP and run `codenotch.exe` with `codenotch-hook.exe` in the same folder. The installer carries both binaries as well.
...
3. **Polling may be paused.** When the usage API answers 429, Codenotch stops calling it for as long as the Retry-After says, and that wait survives a restart. Making the token valid in step 2 and then refreshing is the fastest path.
...
This build includes the upstream fix that renews a nearly expired token by running the standalone `claude` CLI (commit [`27e4b34`](https://github.com/vinzdg/codenotch/commit/27e4b34)), so step 2 is usually unnecessary — only when that CLI cannot be found or the token has already expired.
...
- Setup EXE: `C259AC6B62474670BD4B9A4812153920AA4090528ADA73D81F471285CCB97429`
- ZIP: `332E0178A237B301C950F0F0A1C1295D34C86477D177E3F566931AA886B4B09E`
